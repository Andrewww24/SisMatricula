...
export default async function PlanEstudiantePage({ params }: Params) {
  ...
  const cursos = await db.curso.findMany({
    where: { id_carrera: idCarrera, activo: true },
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <header className="bg-[#0B1F3A] text-white px-6 py-4 flex items-center gap-4">
        <Link href="/plan" className="text-slate-400 hover:text-white text-sm">
          ← Carreras
        </Link>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        {cursos.length === 0 && (
          <div className="bg-white border border-slate-200 rounded-2xl px-6 py-16 text-center text-slate-400 text-sm">
            Esta carrera aún no tiene cursos publicados.
          </div>
        )}
                    ...
                    {lista.map((c) => (
                      <tr key={c.id_curso} className="hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3 text-center text-slate-500 text-xs">
                          {c.pre_requisitos.length > 0
                            ? c.pre_requisitos.map((r) => r.requisito.descripcion).join(", ")
                            : "—"}
                        </td>
                      </tr>
                    ))}
